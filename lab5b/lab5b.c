...
int get_mask(int bits) {
  return power(2, bits) - 1;
}

void pack(int info, int start, int end, int *val) {
  int mask = get_mask(end-start + 1);
  info &= mask;
  info <<= start;
  *val |= info;
}
...
int main()
{
    ...
    */
  char input[50];
  int n = read(0, input, 50);
  InstData data;
  int val = 0b0;
  
  get_inst_data(input, &data);

  pack(data.opcode, 0, 6, &val);

  switch(data.type) {
    case R:
      pack(data.rd, 7, 11, &val);
      pack(data.funct3, 12, 14, &val);
      pack(data.rs1, 15, 19, &val);
      pack(data.rs2, 20, 24, &val);
      pack(data.funct7, 25, 31, &val);
      break;
    ...
    case I:
      ...
      pack(data.funct3, 12, 14, &val);
      pack(data.rs1, 15, 19, &val);
      pack(data.imm, 20, 31, &val);
      break;
            
    case S:
      pack(data.imm, 7, 11, &val);
      data.imm >>= 5;

      pack(data.funct3, 12, 14, &val);
      pack(data.rs1, 15, 19, &val);
      pack(data.rs2, 20, 24, &val);
      pack(data.imm, 25, 31, &val);
      break;
            
    case B:
      data.imm >>= 1;

      pack(data.imm, 8, 11, &val);
      data.imm >>= 4;

      pack(data.imm, 25, 30, &val);
      data.imm >>= 6;

      pack(data.imm, 7, 7, &val);
      data.imm >>= 1;

      pack(data.imm, 31, 31, &val);
      pack(data.funct3, 12, 14, &val);
      pack(data.rs1, 15, 19, &val);
      pack(data.rs2, 20, 24, &val);
      break;
    ...
    case J:
      ...
      pack(data.imm, 21, 30, &val);
      data.imm >>= 10;

      pack(data.imm, 20, 20, &val);
      data.imm >>= 1;

      pack(data.imm, 12, 19, &val);
      data.imm >>= 8;
